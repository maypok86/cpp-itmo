...
private:
    Image m_image;
    size_t CalcHorizontalIndex(size_t index) const;
    size_t CalcVerticalIndex(size_t index) const;
    Seam FindSeam(bool isTranspose) const;

    template <class T, class F>
    void RemoveSeam(size_t width, size_t height, const T & test, const F & getPixel);
};